...
public class Server extends Thread{
	...
	static class ServerThread implements Runnable {
		...
		public void run() {
	        ...
	        try{   
	        	while(true){
	        		if(timeBlockControl!=0){
	        			break;
	        		}
	        		else if(a==1){
	        			break;
	        		}
	        		else if(timeBlockControl==0 && wrongCount <=3){
			            		 ...
			            		 for(int i=0;i<usernamedatabase.length;i++){
			            			 if(line.trim().equals(usernamedatabase[i])){
			            				 ...
			            				 loginCount =1;
			            			 	 ...
			            			 	 break; 	
			            			 } else if( i==usernamedatabase.length-1){
			            				 line ="Try another Username";
										 loginSuccessCount =0; 
			            				 printWriter = new PrintWriter(socket.getOutputStream(),true);
			            				 printWriter.println(line);
			            			 }
								...
								if((line.trim()).equals("whoelse")){
								 ...
								 /*if command is logout ,the client will logout*/
									...
									for(int i=0;i<resultList.length;i++){
										...
										}else if(i==resultList.length-1 &&(resultList[i].equals("message")== false && tempSuccess==false )){
											line ="Wrong Command!!!Re-try!!";
											tempSuccess=false;
										}
